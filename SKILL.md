...
- Dev 自检不能替代独立 Review。当前改动引入、阻断验收、在当前范围存在真实失败路径，或影响当前路由、调用链、副作用及必要安全的 P0-P3 属于可执行问题，必须修复并对新 fixed point 复查；只有能证明与当前交付无关的范围外历史问题和无现实失败路径的理论建议才记录残余风险。可执行问题超出写入授权时暂停受影响节点并请求确认，不擅自扩大范围。
...
5. 严重度按实际影响判断，以下类型不是穷举：P0 为灾难性数据、安全、资金或全局可用性事故；P1 为阻断核心需求、主流程或发布，或造成重大权限、安全、隐私、敏感信息、资金、数据完整性风险；P2 为重要场景的真实正确性、兼容性、可靠性或非阻断安全问题；P3 为低影响但可复现、应在当前范围修复的局部缺陷。当前改动引入、阻断验收、在当前范围存在现实失败路径，或影响当前路由、调用链、副作用及必要安全的问题均为可执行 finding；只有能以 diff 和调用链证明无关的范围外历史问题，或无现实失败路径的理论建议，才可列为残余风险。已经写入的未授权范围扩张按上一条处理。

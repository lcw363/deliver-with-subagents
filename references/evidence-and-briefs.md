...
Reviewer 输入必须固定：`scope_allowlist`、明确不做内容、验收标准、审查范围、base/head SHA 或不可变 snapshot、项目规则、必要测试证据。输出记录 `code_checkpoint` 和审查范围，把当前改动引入、阻断验收、在当前范围存在真实失败路径，或影响当前路由、调用链、副作用及必要安全的问题列为可执行 P0-P3 finding（等级、紧凑行号、失败路径、影响、最小修复建议）；只有能以 diff 和调用链证明与当前交付无关的范围外历史问题，以及无现实失败路径的理论建议，才在必要时列为残余风险。严重度按实际影响判断，权限、安全、隐私、敏感信息、资金和数据完整性问题不得因类型未被枚举而漏报。另行记录范围扩张、测试缺口和 workspace 前后无变化证据；存在未授权范围扩张时不得输出 `SCOPE_OK` 或 `REVIEW_PASS`。没有只读 custom agent 时，前后证据覆盖 HEAD、staged/unstaged patch、范围内 untracked 路径/模式/内容 hash 和 submodule 状态。没有问题时明确写明审查范围、`SCOPE_OK` 与“未发现已知且可执行的 P0-P3 问题”。
